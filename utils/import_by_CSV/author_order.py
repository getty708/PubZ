...
def get_author_orders(url_base, param, logger=None):
    """
    Args.
    -----
    ...
    """
    if logger is None:
        logger = getLogger(__name__ + ".get_author_order")
...
def create_author_order(
    url_base, token, data, logger=getLogger(__name__ + ".create_author_order")
):
    """
    Args.
    -----
    ...
    def _validate():
        key_expected, key_actual = (
            set(["bibtex", "book", "pub_date", "page", "authors"]),
            set(data.keys()),
        )
        if not key_expected == key_actual:
            logger.warning(
                "Check `data`. some keys are missing or it contains unsed keys. [diff={}]".format(
                    key_expected - key_actual
                )
            )
        ...
        def _check(bibtexs):
            ...
            for bib in bibtexs:
                if (bib["title_en"] == bib_title) or (bib["title_ja"] == bib_title):
                    if (
                        (bib["book"]["title"] == book)
                        and (bib["pub_date"] == pub_date)
                        and (bib["page"] == page)
                    ):
                        return bib
            raise ValueError(
                "No bibtex entry was found. [Search Results={}]".format(len(bibtexs))
            )
    ...
    def _get_author(author_name):
        def _check_name(author_name, author_db):
            ...
            if author_name.upper() == name_en:
                ...
                if (_name1[0] == _name2[0]) and (
                    _name1[1] == _name2[1]
                ):  # FamilyName + FirstName
                    return True
                elif (_name1[0] == _name2[1]) and (
                    _name1[1] == _name2[0]
    ...
    for no, author_name in enumerate(authors_list):
        ...
        if r.status_code == 201:
            logger.info(
                "Success: Create new author_order. [Bib{} - {} ({})]".format(
                    bibtex["id"], author_name, no + 1
                )
            )
            ...
            if str(_data) == "['DB IntegrityError']":
                authors_success.append(
                    "{}[{}]".format(author_name, "DB IntegrityError")
                )
                continue
            logger.warning(
                "Failed: Cannot create new author_order. {} \n".format(_data)
            )
...
def main_csv(args):
    ...
    df_success, df_error = (
        df_results[df_results["status"] == True],
        df_results[df_results["status"] == False],
    )
